...
class SampleApp
  ...
  def call(env)
    ...
    when /start\/(.*)$/ then show_template @bundle_template, start_bundle($1)
    when /stop\/(.*)$/ then show_template @bundle_template, stop_bundle($1)
    when /fact\/(\d+)$/ then show_template @result_template, get_factorial($1.to_i)
    when /ruby\/fib\/(\d+)$/ then show_template @result_template, fib($1.to_i)
    when /fib\/(\d+)$/ then show_template @result_template, get_fibonacci($1.to_i)
    else
  ...
  def show_template(page, value)
    [value.nil? ? 204 : 200, {"Content-type" => "text/html", "Cache-Control" => "no-cache"}, page.result(binding) ]
  end
